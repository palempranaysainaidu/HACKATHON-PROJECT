  ...
  return (
    <div className="max-w-3xl mx-auto py-4">
      <div className="mb-8 border-b border-brand-border pb-6">
        <h2 className="font-serif text-3xl font-bold text-brand-white mb-2 flex items-center">
          <Megaphone className="w-8 h-8 mr-3 text-brand-light" /> Broadcast Center
        </h2>
      ...
      <div className="bg-brand-surface border border-brand-border rounded-xl p-6 mb-8">
        <h3 className="font-sans font-bold text-brand-light mb-4">Post a New Update</h3>
        <form onSubmit={handlePostUpdate} className="flex flex-col space-y-4">
          <textarea 
            ...
            className="w-full bg-brand-surface border border-brand-border border border-brand-border p-4 font-sans text-sm focus:outline-none focus:border-brand-accent resize-none"
            required
          />
          <div className="flex justify-between items-center">
            ...
            <button type="submit" className="bg-brand-accent hover:bg-brand-accentHov  text-brand-bg px-8 py-2.5 font-bold font-sans transition-colors">
              Broadcast Now
            </button>
          </div>
        </form>
      ...
      <div className="space-y-4">
        <h3 className="font-sans font-bold text-brand-light mb-4 border-b border-brand-border pb-2">Recent Broadcasts</h3>
        {updates.length === 0 ? (
          <div className="text-center py-10 bg-brand-surface border border-brand-border border border-brand-border rounded-lg text-brand-mid font-sans text-sm">
            No updates broadcasted yet.
          </div>
        ) : (
          updates.map(update => (
            <div key={update._id} className="bg-brand-surface border border-brand-border border text-left border-brand-border p-5 rounded-lg shadow-sm">
              ...
              <p className="font-sans text-sm text-brand-white">{update.content}</p>
            </div>
          ))
        )}
      </div>
