...
export default function AuthPage() {
  ...
  return (
    <div className="w-full flex items-center justify-center min-h-[90vh] px-4 animate-fade-in my-8 font-sans">
      <div className="w-full max-w-5xl bg-brand-surface border border-brand-border border border-brand-border rounded-[2rem] shadow-[0_20px_50px_rgba(0,0,0,0.1)] overflow-hidden flex flex-col md:flex-row">
        
        {/* Left Side: Brand/Marketing */}
        <div className="hidden md:flex w-1/2 bg-brand-dark p-12 text-brand-bg flex-col justify-between relative overflow-hidden group">
        ...
        {/* Right Side: Form */}
        <div className="w-full md:w-1/2 p-10 sm:p-12 bg-brand-surface border border-brand-border relative flex flex-col justify-center">
          <div className="max-w-sm mx-auto w-full">
             
             <div className="mb-10 text-center md:text-left animate-fade-in-up" style={{animationDelay: '0ms'}}>
               <h2 className="text-3xl lg:text-4xl font-serif font-black text-brand-white mb-3 tracking-tight">
             ...
             <form onSubmit={handleSubmit} className="space-y-6">
               {!isLogin && (
                 <div className="animate-fade-in-up" style={{animationDelay: '100ms'}}>
                   ...
                   <input type="text" value={name} onChange={e => setName(e.target.value)} required className="w-full bg-brand-surface border border-brand-border rounded-xl px-4 py-3.5 font-sans text-base text-brand-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-brand-dark focus:bg-brand-surface border border-brand-border transition-all duration-300 shadow-sm hover:shadow-md" placeholder="e.g. Director Sarah" />
                 </div>
               )}
               ...
               <div className="animate-fade-in-up" style={{animationDelay: isLogin ? '100ms' : '200ms'}}>
                 ...
                 <input type="email" value={email} onChange={e => setEmail(e.target.value)} required className="w-full bg-brand-surface border border-brand-border rounded-xl px-4 py-3.5 font-sans text-base text-brand-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-brand-dark focus:bg-brand-surface border border-brand-border transition-all duration-300 shadow-sm hover:shadow-md" placeholder="hello@domain.com" />
               </div>
               ...
               <div className="animate-fade-in-up" style={{animationDelay: isLogin ? '200ms' : '300ms'}}>
                 <div className="flex justify-between items-center mb-2.5">
                   ...
                   {isLogin && <button type="button" className="text-[11px] font-bold text-brand-light hover:text-black transition-colors hover:underline tracking-widest uppercase">Recover?</button>}
                 </div>
                 <input type="password" value={password} onChange={e => setPassword(e.target.value)} required className="w-full bg-brand-surface border border-brand-border rounded-xl px-4 py-3.5 font-sans text-base text-brand-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-brand-dark focus:bg-brand-surface border border-brand-border transition-all duration-300 shadow-sm hover:shadow-md" placeholder="••••••••" />
               </div>
               ...
               {!isLogin && (
                 <div className="animate-fade-in-up" style={{animationDelay: '400ms'}}>
                   ...
                   <div className="relative">
                     <select value={role} onChange={e => setRole(e.target.value)} className="w-full appearance-none bg-brand-surface border border-brand-border rounded-xl px-4 py-3.5 font-sans text-base font-medium text-brand-white focus:outline-none focus:ring-2 focus:ring-brand-dark focus:bg-brand-surface border border-brand-border transition-all duration-300 shadow-sm cursor-pointer">
               ...
               <button type="submit" disabled={loading} className="w-full bg-brand-accent hover:bg-brand-accentHov  text-brand-bg font-sans font-bold text-base px-6 py-4 rounded-xl transition-all duration-300 mt-8 flex items-center justify-center group disabled:opacity-70 disabled:cursor-not-allowed shadow-lg shadow-black/15 animate-fade-in-up hover:-translate-y-1" style={{animationDelay: isLogin ? '300ms' : '500ms'}}>
             ...
             <div className="mt-10 text-center animate-fade-in" style={{animationDelay: isLogin ? '400ms' : '600ms'}}>
               <button onClick={() => setIsLogin(!isLogin)} className="text-sm text-brand-mid hover:text-brand-white transition-colors font-bold font-sans border-b-2 border-transparent hover:border-brand-accent pb-1 uppercase tracking-widest">
