  ...
  if (mode === 'select') {
    return (
      <div className="max-w-4xl mx-auto h-[70vh] flex flex-col items-center justify-center animate-fade-in-up">
        <h1 className="font-serif text-4xl font-black text-brand-black mb-4">How would you like to build?</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
          <button onClick={() => setMode('ai')} className="group flex flex-col items-center justify-center p-10 bg-brand-surface border-2 border-transparent hover:border-brand-info hover:bg-white transition-all shadow-sm rounded-none text-center">
             <div className="w-16 h-16 bg-white border border-brand-border rounded-none flex items-center justify-center mb-6 group-hover:scale-110 shadow-sm transition-transform">
               <Bot className="w-8 h-8 text-brand-info" />
             </div>
             <h3 className="font-serif text-2xl font-bold text-brand-black mb-2">✨ Full AI Genesis</h3>
          ...
          <button onClick={() => setMode('manual')} className="group flex flex-col items-center justify-center p-10 bg-brand-surface border-2 border-transparent hover:border-brand-black hover:bg-white transition-all shadow-sm rounded-none text-center">
             <div className="w-16 h-16 bg-white border border-brand-border rounded-none flex items-center justify-center mb-6 group-hover:scale-110 shadow-sm transition-transform text-brand-black font-serif text-2xl font-bold">
               M
             </div>
             <h3 className="font-serif text-2xl font-bold text-brand-black mb-2">Manual Configuration</h3>
  ...
  if (mode === 'ai') {
    return (
      <div className="max-w-3xl mx-auto animate-fade-in-up mt-10">
        <div className="mb-8">
          <button onClick={() => setMode('select')} className="text-brand-mid hover:text-brand-black text-sm font-bold uppercase tracking-widest font-sans mb-6 inline-block">&larr; Back to Modes</button>
          <h1 className="font-serif text-4xl font-black text-brand-black mb-2 flex items-center">
          ...
          {!loading ? (
            <>
              <textarea 
                ...
                className="w-full h-48 border border-brand-border bg-brand-surface p-5 font-sans text-brand-black placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-brand-info resize-none mb-6 text-lg leading-relaxed shadow-inner"
              ></textarea>
              
              <div className="border-t border-brand-border pt-6 mb-6">
                <h3 className="font-serif font-bold text-lg mb-4 text-brand-black">Base Configuration</h3>
                <div className="grid md:grid-cols-3 gap-6">
                  ...
                  <div>
                    ...
                    <input type="number" min="0" value={aiConfig.ticketPrice} onChange={e => setAiConfig({...aiConfig, ticketPrice: e.target.value})} className="w-full border border-brand-border px-3 py-2 text-sm focus:outline-none focus:border-brand-black" placeholder="0 = Free" />
                  </div>
                </div>
              </div>
              ...
              <div className="flex justify-end">
                <button 
                  ...
                  className="bg-brand-black hover:bg-black text-white px-10 py-4 font-sans font-bold shadow-md hover:-translate-y-1 transition-transform flex items-center group cursor-pointer"
                >
            ...
            <div className="h-64 flex flex-col items-center justify-center space-y-6">
              ...
              <div className="font-mono text-xs uppercase tracking-[0.2em] text-brand-dark animate-pulse">{aiStatus}</div>
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <div className="max-w-3xl mx-auto animate-fade-in-up mt-10">
      <div className="mb-8 flex justify-between items-end">
        <div>
          <button onClick={() => setMode('select')} className="text-brand-mid hover:text-brand-black text-sm font-bold uppercase tracking-widest font-sans mb-6 inline-block">&larr; Back</button>
          <h1 className="font-serif text-3xl font-bold text-brand-black mb-2">Create New Event</h1>
          <div className="flex items-center space-x-2 text-sm font-sans text-brand-mid">
            <span className={step >= 1 ? 'font-bold text-brand-black' : ''}>1. Details</span>
            <span>&rarr;</span>
            <span className={step >= 2 ? 'font-bold text-brand-black' : ''}>2. Logistics</span>
            <span>&rarr;</span>
            <span className={step >= 3 ? 'font-bold text-brand-black' : ''}>3. Review</span>
          </div>
        </div>
      </div>
        ...
        {step === 1 && (
          <div className="space-y-5">
            ...
            <div>
              ...
              <input type="text" name="name" value={formData.name} onChange={handleChange} className="w-full bg-brand-surface border border-brand-border px-4 py-3 font-sans focus:outline-none focus:border-brand-black" placeholder="Annual Tech Fest 2025" />
            </div>
            
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-bold uppercase tracking-wide text-brand-mid mb-2">Event Type</label>
                <select name="type" value={formData.type} onChange={handleChange} className="w-full bg-brand-surface border border-brand-border px-4 py-3 font-sans focus:outline-none focus:border-brand-black">
              ...
              <div>
                ...
                <input type="date" name="date" value={formData.date} onChange={handleChange} className="w-full bg-brand-surface border border-brand-border px-4 py-3 font-sans focus:outline-none focus:border-brand-black" />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" name="city" value={formData.city} onChange={handleChange} className="w-full bg-brand-surface border border-brand-border px-4 py-3 font-sans focus:outline-none focus:border-brand-black" placeholder="Mumbai" />
              </div>
              <div>
                <label className="block text-xs font-bold uppercase tracking-wide text-brand-mid mb-2">Expected Audience</label>
                <input type="number" name="expectedAudience" value={formData.expectedAudience} onChange={handleChange} className="w-full bg-brand-surface border border-brand-border px-4 py-3 font-sans focus:outline-none focus:border-brand-black" placeholder="500" />
              </div>
            </div>

            <div>
              <label className="block text-xs font-bold uppercase tracking-wide text-brand-mid mb-2">Description</label>
              <textarea name="description" value={formData.description} onChange={handleChange} rows="4" className="w-full bg-brand-surface border border-brand-border px-4 py-3 font-sans focus:outline-none focus:border-brand-black" placeholder="Describe the vision..."></textarea>
            </div>

            <div className="flex justify-end pt-6">
              <button onClick={nextStep} className="bg-brand-black hover:bg-black text-white font-sans font-bold px-8 py-3 transition-colors">
                Next Step
              </button>
            </div>
          </div>
        ...
        {step === 2 && (
          <div className="space-y-5">
            ...
            <div>
              ...
              <input type="number" name="totalBudget" value={formData.totalBudget} onChange={handleChange} className="w-full bg-brand-surface border border-brand-border px-4 py-3 font-sans focus:outline-none focus:border-brand-black" />
            </div>

            <div>
              <label className="block text-xs font-bold uppercase tracking-wide text-brand-mid mb-2">Number of Volunteers Needed</label>
              <input type="number" name="volunteersNeeded" value={formData.volunteersNeeded} onChange={handleChange} className="w-full bg-brand-surface border border-brand-border px-4 py-3 font-sans focus:outline-none focus:border-brand-black" />
            </div>
            ...
            <div className="flex items-center space-x-3 pt-4 border-t border-brand-border">
              ...
              <span className="font-sans text-brand-dark font-medium">Will you provide food for attendees?</span>
            </div>
            ...
            <div className="flex items-center space-x-3">
              ...
              <span className="font-sans text-brand-dark font-medium">Will you provide transportation for attendees?</span>
            </div>

            <div className="flex justify-between pt-6 mt-4 border-t border-brand-border">
              <button onClick={prevStep} className="border border-brand-border text-brand-dark font-sans font-bold px-8 py-3 hover:bg-brand-surface">
                Back
              </button>
              <button onClick={nextStep} className="bg-brand-black hover:bg-black text-white font-sans font-bold px-8 py-3">
        ...
        {step === 3 && (
          <div className="space-y-6">
            ...
            <div className="bg-brand-surface p-6 border border-brand-border">
              <h3 className="font-serif text-3xl font-bold text-brand-black mb-4">{formData.name || 'Untitled Event'}</h3>
            ...
            <div className="flex justify-between pt-6 border-t border-brand-border">
              ...
              <button onClick={handleManualSubmit} disabled={loading} className="bg-brand-black hover:bg-black text-white font-sans font-bold px-10 py-3 flex justify-center w-48 shadow-md hover:-translate-y-0.5 transition-transform disabled:opacity-50">
