...
export default function VolunteerWorkspace() {
  ...
  return (
    <div className="max-w-7xl mx-auto">
      <div className="border-b border-brand-border pb-6 mb-8 flex justify-between items-center bg-brand-white p-6 rounded-xl border">
         <div>
           ...
           <h1 className="font-serif text-3xl font-bold text-brand-black">Volunteer Operations Center</h1>
         </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
         <div className="lg:col-span-2 space-y-8">
            <section className="bg-brand-white border border-brand-border rounded-xl p-6 flex flex-col h-[400px]">
               <h2 className="font-sans font-bold text-lg text-brand-dark border-b border-brand-border pb-3 mb-4">My Assigned Tasks</h2>
               {tasks.length === 0 ? (
                 ...
                 <div className="space-y-4 overflow-y-auto pr-2 flex-1">
                    {tasks.map(task => (
                      <div key={task._id} className="border border-brand-border rounded-lg p-4 flex justify-between items-center group hover:border-brand-mid transition-colors flex-shrink-0">
                         <div>
                            <h3 className="font-sans font-semibold text-brand-black">{task.title}</h3>
                            <p className="font-sans text-xs text-brand-mid mt-1">{task.description}</p>
                         </div>
                         <select 
                           value={task.status} 
                           onChange={e => handleStatusChange(task._id, e.target.value)}
                           className={`font-sans text-xs font-semibold px-3 py-1.5 rounded-full border ${task.status === 'completed' ? 'bg-brand-tag text-brand-mid border-transparent' : 'bg-brand-white border-brand-border text-brand-dark'}`}
                         >
            ...
            <section className="bg-brand-white border border-brand-border rounded-xl p-6">
               <div className="flex items-center space-x-2 mb-4">
                 <Camera className="w-5 h-5 text-brand-black" />
                 <h2 className="font-sans font-bold text-lg text-brand-dark">Attendee Check-In Station</h2>
               </div>
               ...
               <form onSubmit={handleMarkAttendance} className="flex space-x-3">
                 <input 
                   ...
                   maxLength={8}
                 />
                 <button type="submit" className="bg-brand-black hover:bg-black text-white px-8 py-3 rounded-lg font-sans font-medium transition-colors">
                   Verify
                 </button>
               </form>
            </section>
         ...
         <div className="lg:col-span-1 space-y-8">
            <section>
               <h2 className="font-sans font-bold text-lg text-brand-dark mb-4">Team Comms</h2>
               <EventChat eventId={id} />
            </section>

            <section className="bg-brand-surface border border-brand-border rounded-xl p-6 flex flex-col h-[400px]">
               <h2 className="font-sans font-bold text-lg text-brand-dark border-b border-brand-border pb-3 mb-4">Live Updates Feed</h2>
               
               <form onSubmit={handlePostUpdate} className="flex space-x-2 mb-4">
                 <input type="text" value={newUpdate} onChange={e=>setNewUpdate(e.target.value)} placeholder="e.g. Food running low!" className="flex-1 text-sm px-3 py-2 border border-brand-border rounded outline-none focus:border-brand-black" />
                 <button type="submit" className="bg-brand-black text-white px-3 py-2 text-sm font-bold rounded">Post</button>
               </form>
               ...
               <div className="overflow-y-auto flex-1">
                 {updates.length === 0 ? (
                   ...
                   {updates.map(update => (
                     <div key={update._id} className="bg-brand-white p-3 rounded-lg shadow-sm border border-brand-border">
                       <span className="block text-[10px] font-bold uppercase tracking-wider text-brand-info mb-1">{update.type}</span>
                       <p className="font-sans text-sm text-brand-dark">{update.content}</p>
